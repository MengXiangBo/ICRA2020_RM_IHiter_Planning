...
void Chassis::ROS_Init(){
  //out.open("/home/dengxin/chassis_angle.txt");
  //ros publisher
  ros_odom_pub_ = ros_nh_.advertise<nav_msgs::Odometry>("odom", 1);
...
void Chassis::ChassisInfoCallback(const std::shared_ptr<roborts_sdk::cmd_chassis_info> chassis_info){
  ...
  //ROS_INFO("angle = %f, x = %f, y = %f", float(chassis_info->gyro_angle), float(chassis_info->position_x_mm), float(chassis_info->position_y_mm));
  //out << chassis_info->gyro_angle << std::endl;
...
void Chassis::ChassisSpeedCtrlCallback(const geometry_msgs::Twist::ConstPtr &vel){
  ...
  //ROS_INFO("vx = %f", float(chassis_speed.vx));
  //ROS_INFO("vy = %f", float(chassis_speed.vy));
